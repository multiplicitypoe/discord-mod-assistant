...
def analyze_incident(
    ...
    prompt = (
        ...
        "- summary: 1-2 sentences, <= 220 chars, plain language a moderator would use. "
        "It must stand on its own, because it is the first thing read: "
        "say what happened, who did it, and where it stands now. Do not describe the "
        "channel being normal beforehand unless that actually matters. Do not use bold: "
        "it is rendered after a bold label and more bold would compete with it.\n"
        "  Name what people actually did, exactly. Write 'pinged @Chat Moderator', not "
        "'asked for a moderator'. Write 'posted a link to a key reselling site', not "
        "'shared something suspicious'. A vaguer word than the thing itself is always "
        "wrong, because the moderator has to go and look to find out what you meant.\n"
        "- participants: <= 4\n"
        "- signals: <= 4\n"
        "- rule_refs: <= 2\n"
