...
def analyze_incident(
    ...
    prompt = (
        ...
        "Do NOT include the recommended action: it is rendered separately and repeating "
        "it wastes the line. No names, no @mentions, no rule ids, no trailing period.\n"
        "- summary: 1-2 sentences, <= 200 chars, plain language a moderator would use. "
        "It must stand on its own, because it is the first thing read: "
        "say only what the offender did. Never mention that someone pinged, summoned, "
        "or reported a moderator - that is what triggered this brief and is already "
        "implied by it existing, so saying it again just wastes the line. Do not "
        ...
        "  Name what people actually did, exactly. Write 'posted a link to a key "
        ...
        "- signals: <= 4\n"
        "- rule_refs: <= 2\n"
        "- recommendations: the action(s) to take, usually exactly one short phrase "
        "combining what's needed, e.g. 'Remove the post and warn', 'Remove the post "
        "and ban', 'Time out the user', 'Time out both users'. A second item only for "
        "a genuinely separate action against a different person. Do not cite rule ids "
        "here, rule_refs already carries that. If none: [\\\"No action.\\\"].\n"
